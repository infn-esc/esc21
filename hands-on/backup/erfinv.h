...
inline float unsafe_logf_impl(float x)
{
  ...
  m.i32 =
      (xx.i32 & 0x007FFFFF) | 0x3F800000; // extract mantissa as an FP number

  int adjust =
      (xx.i32 >> 22) & 1; // first bit of the mantissa, tells us if 1.m > 1.5
  m.i32 -=
      adjust << 23; // if so, divide 1.m by 2 (exact operation, no rounding)
  e += adjust;      // and update exponent so we still have x=2^E*y

  // now back to floating-point
  float y = m.f - 1.0f; // Sterbenz-exact; cancels but we don't care about
...
inline float erfinv_like(float w)
{
  w       = w - 2.500000f;
...
inline float erfinv_unlike(float w)
{
...
inline void erfinV(float* __restrict__ x, float const* __restrict__ ur,
                   uint32_t N)
{
