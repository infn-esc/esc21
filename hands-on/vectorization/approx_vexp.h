...
Feel free to distribute or insert in other programs etc, as long as this notice
is attached. Comments, requests etc: Florent.de.Dinechin@ens-lyon.fr

Polynomials were obtained using Sollya scripts (in comments):
please also keep these comments attached to the code.

If a derivative of this code ends up in the glibc I am too happy: the version
with MANAGE_SUBNORMALS=1 and DEGREE=6 is faithful-accurate over the full 2^32
binary32 numbers and behaves well WRT exceptional cases. It is about 4 times
faster than the stock expf on this PC, when compiled with gcc -O2.

This code is FMA-safe (i.e. accelerated and more accurate with an FMA) as long
as my parentheses are respected.

A remaining TODO is to try and manage the over/underflow using only integer
tests as per Astasiev et al, RNC conf. Not sure it makes that much sense in the
vector context.

*/

// #define MANAGE_SUBNORMALS 1 // No measurable perf difference, so let's be
// clean. If set to zero we flush to zero the subnormal outputs, ie for x<-88 or
// so
...
// (I don't subtract the loop and call overhead, so it would be more for inlined
// code)
...
struct approx_expf_P
{
  static Float impl(Float p);
};
...
struct approx_expf_P<Float, 3>
{
  static Float impl(Float y)
  {
    ...
    return float(0x2.p0)
         + y
               * (float(0x1.fff798p0)
                  + y * (float(0x1.02249p0) + y * float(0x5.62042p-4)));
#else // ESTRIN
...
struct approx_expf_P<Float, 6>
{
  static Float impl(Float y)
  {
    ...
    Float y2  = y * y;
    Float p12 =
        float(0x2.p0) + y; // By chance we save one operation here! Funny.
...
inline Float __attribute__((always_inline)) approx_expf(Float x)
{
  ...
  constexpr Float zero_threshold_ftz =
      zero - float(0x5.75628p4); // sollya: single(log(1b-126));
  // flush to zero on the output
  // manage infty output:
  // faster than directly on output!
  // using std::min; using std::max;
  using nativeVector::max;
  using nativeVector::min;
