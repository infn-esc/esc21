...
struct NeuNet
{
  ...
  void train(std::array<T, N> const& x, T const& target, float learingRate)
  {
    ...
    for (int i = 0; i < M; ++i)
      hidden2.neurons[i].error = output.w[i] * output.error;
    for (int i = 0; i < M; ++i)
      hidden1.neurons[i].error = 0;
    for (int j = 0; j < M; ++j)
      for (int i = 0; i < M; ++i)
        hidden1.neurons[j].error +=
            hidden2.neurons[i].w[j] * hidden2.neurons[i].error;
...
void go()
{
  ...
  for (auto& w : net.output.w)
    w = wgen(eng);
  for (auto& n : net.hidden2.neurons)
    for (auto& w : n.w)
      w = wgen(eng);
  for (auto& n : net.hidden1.neurons)
    for (auto& w : n.w)
      w = wgen(eng);
