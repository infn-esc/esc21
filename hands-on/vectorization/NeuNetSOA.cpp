...
#include <random>
#include <vector>
// c++ -Ofast -fopenmp -mavx2 -mfma NeuNetNaive.cpp  -fopt-info-vec
  ...
  void updateWeight(float learingRate)
  {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      for (auto j = 0U; j < VSIZE; ++j)
        w[i] += tmp[j] / float(VSIZE);
    }
    for (auto j = 0U; j < VSIZE; ++j)
      w[N] += corr[j] / float(VSIZE);
  }
...
struct NeuNet
{
  ...
  void train(std::array<T, N> const& x, T const& target, float learingRate)
  {
    ...
    for (int i = 0; i < M; ++i)
      middle.neurons[i].error = output.w[i] * output.error;
    for (int i = 0; i < M; ++i)
      input.neurons[i].error = vzero;
    for (int j = 0; j < M; ++j)
      for (int i = 0; i < M; ++i)
        input.neurons[j].error +=
            middle.neurons[i].w[j] * middle.neurons[i].error;
...
struct Reader
{
  explicit Reader(long long iread)
      : toRead(iread)
  {}
...
void go()
{
  ...
  // auto vgen = [&]()->FVect { return
  // FVect{wgen(eng),wgen(eng),wgen(eng),wgen(eng),
  // wgen(eng),wgen(eng),wgen(eng),wgen(eng)}; };

  NeuNet<FVect, NX, MNodes> net;

  for (auto& w : net.output.w)
    w = wgen(eng);
  for (auto& n : net.middle.neurons)
    for (auto& w : n.w)
      w = wgen(eng);
  for (auto& n : net.input.neurons)
    for (auto& w : n.w)
      w = wgen(eng);
  ...
  Data buffer;
  for (auto& b : buffer)
    posix_memalign((void**)(&b), sizeof(FVect), sizeof(float) * bufSize);
