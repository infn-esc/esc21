...
struct approx_logf_P<Float, 3>
{
  static Float impl(Float y)
  {
    return y
         * (float(0x1.013354p0)
            + y * (-float(0x8.33006p-4) + y * float(0x4.0d16cp-4)));
  }
};
  ...
  static Float impl(Float y)
  {
    return y
         * (float(0xf.ff5bap-4)
            + y
                  * (-float(0x8.13e5ep-4)
                     + y * (float(0x5.826ep-4) + y * (-float(0x2.e87fb8p-4)))));
  }
};
...
inline Float __attribute__((always_inline)) unsafe_logf_impl(Float x)
{
  ...
  // as many integer computations as possible, most are 1-cycle only, and lots
  // of ILP.
  Int e = (((ix) >> 23) & 0xFF) - 127;    // extract exponent
  Int m = (ix & 0x007FFFFF) | 0x3F800000; // extract mantissa as an FP number

  Int adjust =
      (ix >> 22) & 1;  // first bit of the mantissa, tells us if 1.m > 1.5
  ...
  Float y = toIF<Float>::itof(m) - 1.0f; // Sterbenz-exact; cancels but we don't
                                         // care about output relative error
