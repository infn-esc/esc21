...
struct VType
{
  static constexpr auto elem(V x) ->
...
struct VType<float>
{
  static constexpr auto elem(float x) -> float
  {
    return x;
  }
};
...
struct VType<double>
{
  static constexpr auto elem(double x) -> double
  {
    return x;
  }
};
...
struct ConvertVector<float32x4_t>
{
  ...
  static VF impl(itype i)
  {
    return VF(_mm_cvtepi32_ps(__m128i(i)));
  }
  static itype impl(VF f)
  {
    return itype(_mm_cvttps_epi32(__m128(f)));
  }
};
...
struct ConvertVector<float32x8_t>
{
  ...
  static VF impl(itype i)
  {
    return VF(_mm256_cvtepi32_ps(__m256i(i)));
  }
  static itype impl(VF f)
  {
    return itype(_mm256_cvtps_epi32(__m256(f)));
  }
};
#endif
...
struct toIF
{
  ...
  static itype ftoi(VF f)
  {
    itype i;
    memcpy(&i, &f, NV);
    return i;
  }
  static VF itof(itype i)
  {
    VF f;
    memcpy(&f, &i, NV);
    return f;
  }
  static uitype ftoui(VF f)
  {
    uitype i;
    memcpy(&i, &f, NV);
    return i;
  }
  static VF uitof(uitype i)
  {
    VF f;
    memcpy(&f, &i, NV);
    return f;
  ...
  static VF convert(itype i)
  {
    return ConvertVector<VF>::impl(i);
  }
  static itype convert(VF f)
  {
    return ConvertVector<VF>::impl(f);
  }
};
...
IVect convert(FVect v)
{
  return toIF<FVect>::convert(v);
}
FVect convert(IVect v)
{
  return toIF<FVect>::convert(v);
}
...
#include <iostream>
inline std::ostream& operator<<(std::ostream& co, nativeVector::FVect const& v)
{
  co << '(' << v[0];
  for (unsigned int i = 1; i < nativeVector::VSIZE; ++i)
    co << ',' << v[i];
  return co << ')';
}
inline std::ostream& operator<<(std::ostream& co, nativeVector::IVect const& v)
{
  co << '(' << v[0];
  for (unsigned int i = 1; i < nativeVector::VSIZE; ++i)
    co << ',' << v[i];
